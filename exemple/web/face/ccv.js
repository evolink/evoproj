				...
				for (q = 0; q < 4; q++) {
					...
					for (y = 0; y < qh; y++) {
						for (x = 0; x < qw; x++) {
							...
							if (flag) {
								seq.push({"x" : (x * 4 + dx[q] * 2) * scale_x - cascade.width * scale_x * 0.125,
										  "y" : (y * 4 + dy[q] * 2) * scale_y - cascade.height * scale_y * 0.325,
										  "width" : cascade.width * scale_x * 1.25,
										  "height" : cascade.height * scale_y * 1.65,
										  "neighbor" : 1,
										  "confidence" : sum});
							}
							u8o[0] += 16;
